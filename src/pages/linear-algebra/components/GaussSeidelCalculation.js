export const calculateGaussSeidel = (matrixA, matrixB, initialX, dimension) => {
  ...
  for (let k = 0; k < maxIterations; k++) {
    ...
    for (let i = 0; i < dimension; i++) {
      ...
      for (let j = 0; j < dimension; j++) {
        if (j !== i) {
          // Use updated values as soon as they're available
          sum += matrixA[i][j] * (j < i ? nextX[j] : currentX[j]);
        }
      }
